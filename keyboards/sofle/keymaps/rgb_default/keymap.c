 /* Copyright 2022 Nicholas Hammond
  *
...
const rgblight_segment_t PROGMEM rgblayer_lower[] = RGBLIGHT_LAYER_SEGMENTS( { 0, 58, HSV_GREEN  } );
const rgblight_segment_t PROGMEM rgblayer_raise[] = RGBLIGHT_LAYER_SEGMENTS( { 0, 58, HSV_PURPLE } );
...
static void left_oled(void) {
    ...
    oled_write_P(PSTR("\n"), false);
    oled_write_ln_P(PSTR("Aura's\nKeeb"), false);

	// Print current layer
	oled_write_ln_P(PSTR("LAYER"), false);
	switch (toplayer) {
		case _QWERTY: oled_write_P(PSTR(" <3  "), false); break;
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    switch (get_highest_layer(layer_state)) {
        ...
        default:	  encoder_layered_none (index, clockwise); break;
    }
    return true;
}
#endif
